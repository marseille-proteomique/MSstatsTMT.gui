...
#' dataProcessPlotsTMT takes the quantitative data from converter functions (\code{\link{PDtoMSstatsTMTFormat}},
#' \code{\link{MaxQtoMSstatsTMTFormat}}, \code{\link{SpectroMinetoMSstatsTMTFormat}}) as input
#' and generate profile plot, to identify the potential sources of variation for each protein
#'
#'
#' @import ggplot2
...
#'
#' ## Profile plot
#' PlotsTMT_profile(data.peptide = input.pd,
#'                  data.summarization = quant.msstats,
#'                  which.Protein = "Q9UGP8", summaryPlot = FALSE)

PlotsTMT_profile <- function(data.peptide,
                             data.summarization,
                             ylimUp = FALSE,
                             ylimDown = FALSE,
                             x.axis.size = 10,
  ...
  if (sum(is.element(colnames(datarun), "Run")) != 0) {
    datamat <- reshape2::dcast(Protein + Channel ~ Run, data = datarun, value.var = 'Abundance', keep = TRUE)

    datarun <- reshape2::melt(datamat, id.vars=c('Protein', 'Channel'))
  ...
  if (originalPlot & !summaryPlot){
    return(ptemp)
  }
  if (summaryPlot & !originalPlot){
    return(ptempall)
  }
  if (summaryPlot & originalPlot){
    return(ggarrange(ptemp, ptempall, nrow = 2, ncol = 1, heights = c(1.5,1)))
  }
